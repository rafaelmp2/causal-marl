...
class CommAgents:
	...
	def choose_action_comm_qmix(self, obs, last_action, agent_num, weights, avail_actions, epsilon, evaluate=None):
		inputs = obs.copy()
		avail_actions_ind = np.nonzero(avail_actions)[0]  # index of actions which can be choose
		...
		if self.args.alg == 'qmix+commnet':
			#lb_weight = min(weights)
			#norm_weights = weights + abs(lb_weight) + 1
			#q_value = q_value * norm_weights

			#q_value = q_value + weights # seems a good step

			#normalize tensors of q_values and weights
			min_q_values = torch.min(q_value, dim=1)[0]
			max_q_values = torch.max(q_value, dim=1)[0]
			norm_q_values = (q_value - min_q_values) / (max_q_values - min_q_values)

			min_weights = torch.min(weights, dim=0)[0]
			max_weights = torch.max(weights, dim=0)[0]
			norm_weights = (weights - min_weights) / (max_weights - min_weights)

			# now decides if use commnet or just q_value from qmix
			best_q = torch.max(norm_q_values)

			norm_weighted_q_values = norm_weights * norm_q_values
			best_weighted_q = torch.max(norm_weighted_q_values)

			#print("norm_q_values\n",norm_q_values)
			#print("norm_weights\n", norm_weights)
			#print("norm_weighted_q_values\n", norm_weighted_q_values)
			#print("best_q: ", best_q, "best_weighted_q: ", best_weighted_q)

			if best_q < best_weighted_q:
				q_value = norm_weighted_q_values

			#print("final q_values: ", q_value, "\n")
	...
	def get_action_weights(self, obs, last_action):
		...
		if self.args.alg == 'qmix+commnet':
			weights, self.policy.eval_comm_hidden = self.policy.eval_comm(inputs, self.policy.eval_comm_hidden)
