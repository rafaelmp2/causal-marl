...
class ACD_marl:
	def __init__(self, args):
		...
		print("IDQL_ACD algorithm initialized")


	def learn(self, batch, max_episode_len, train_step, agent_id, causality_values, r_idxs, epsilon=None):  
		...
		obs, obs_next, actions, reward, avail_actions, avail_actions_next, terminated = batch['obs'], \
		                          	batch['obs_next'], batch['actions'][:, :, agent_id], batch['reward'], \
		                            batch['avail_actions'][:, :, agent_id], batch['avail_actions_next'][:, :, agent_id], batch['terminated']


		# reward to be adjusted
		reward_aux = reward.cuda()

		# address credit assignment on the rewards
		if len(r_idxs) > 0:
			causal_values = causality_values[:, agent_id]
			for k, ep in enumerate(r_idxs):
				reward_aux[ep, (reward_aux[ep] > 0.).squeeze(1).nonzero().cuda()] *= causal_values[k]  
		...
		# changed to give rewards with causal factor
