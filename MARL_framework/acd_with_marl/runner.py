...
import numpy as np
import torch
from common.acd_estimator import ACD_estimator
...
class Runner:
	def __init__(self, env, args):
		...
		# causal estimator
		if self.args.alg == 'acd_marl':
			self.causal_estimator = ACD_estimator(args)
		...
		while time_steps < self.args.n_steps:
			...
			if self.args.alg.find('coma') > -1:
				...
				for train_step in range(self.args.train_steps):
					...
					# for the causality estimations for a mini batch of episodes
					causality_values = None
					r_idxs = []
					if self.args.alg == 'acd_marl':
						causality_values, r_idxs = self.causal_estimator.estimate_causality(mini_batch)

					for agent_id in range(self.args.n_agents):
						self.list_agents[agent_id].train(mini_batch, train_steps, agent_id, causality_values, r_idxs) 
					train_steps += 1
