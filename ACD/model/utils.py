...
# debug: to print_matrix
def print_matrix_from_array(arr, n, x):
    l = []
    k = 0
    mat = np.eye(x, x)
    for i in range(x):
...
def edge_accuracy(preds, target, binary=True, epoch=None, batch_idx=None):
    """Based on https://github.com/ethanfetaya/NRI (MIT License)."""
    _, preds = preds.max(-1)
    
    # just to save files for additional metrics
    #if epoch != None and epoch % 10 == 0:
    #    if epoch == 150:
    #        np.save(f"pred_edges_pp5p1p5533_at_{epoch}_epochs_batch_{batch_idx}.npy", preds.cpu().numpy())
    #        np.save(f"target_edges_pp5p1p5533_at_{epoch}_epochs_batch_{batch_idx}.npy", target.cpu().numpy())
    #    else:    
    #        np.save(f"pred_edges_pp5p1p5533_at_{epoch}_epochs.npy", preds.cpu().numpy())
    #        np.save(f"target_edges_pp5p1p5533_at_{epoch}_epochs.npy", target.cpu().numpy())
