...
def load_data(args):
    ...
    elif "PredatorPrey" in args.suffix:
        suff = 'pp'  
        train_loader, valid_loader, test_loader = load_pp_data(
            args,
            suffix=suff,
            batch_size=args.batch_size_multiGPU,
            datadir=args.datadir,
        )
    elif "3m" in args.suffix:
        suff = "3m" 
        train_loader, valid_loader, test_loader = load_smac_data(
            args,
            suffix=suff,
            batch_size=args.batch_size_multiGPU,
            datadir=args.datadir,
        )
    elif "Lumberjacks" in args.suffix:
        suff = 'lj'  
        train_loader, valid_loader, test_loader = load_lj_data(
            args,
            suffix=suff,
            batch_size=args.batch_size_multiGPU,
            datadir=args.datadir,
...
def load_pp_data(args, batch_size=1, suffix="", datadir="data"):

    # load files
    obs_arr = np.load(datadir + "/obs_filt_" + suffix + "_sample.npy")[:, :, :, 2:27]
    rewards_arr = np.load(datadir + "/r_filt_" + suffix + "_sample.npy")
    edges_file = np.load(datadir + "/gt_edges_" + suffix + "_sample.npy")

    edges_train, edges_test, edges_valid = [], [], []

    train, test, valid = [], [], []
    obs_arr = np.transpose(obs_arr, [0, 2, 1, 3])
    
    rewards_arr = rewards_arr.reshape(-1, 1, rewards_arr.shape[1], rewards_arr.shape[2])
    
    # zero pad
    rewards_arr = np.pad(rewards_arr, ((0, 0), (0, 0), (0, 0), (0, 24)))

    # [bs, ts, n_agents, n_obs]
    obs_rewards_concatenate_ts = np.concatenate((obs_arr, rewards_arr), axis=1)

    # 80% - 10% - 10%; adjust as needed
    train = np.array(obs_rewards_concatenate_ts[:int(len(obs_arr)*0.8)])
    valid = np.array(obs_rewards_concatenate_ts[int(len(obs_arr)*0.8):int(len(obs_arr)*0.9)])
    test = np.array(obs_rewards_concatenate_ts[int(len(obs_arr)*0.9):int(len(obs_arr))])

    # 80% - 10% - 10%; adjust as needed
    edges_train = np.array(edges_file[:int(len(obs_arr)*0.8)]).reshape(-1, args.num_atoms*args.num_atoms)
    edges_valid = np.array(edges_file[int(len(obs_arr)*0.8):int(len(obs_arr)*0.9)]).reshape(-1, args.num_atoms*args.num_atoms)
    edges_test = np.array(edges_file[int(len(obs_arr)*0.9):int(len(obs_arr))]).reshape(-1, args.num_atoms*args.num_atoms)
    ...
    train_data = TensorDataset(train, edges_train)
    valid_data = TensorDataset(valid, edges_valid)
    test_data = TensorDataset(test, edges_test)
...
def load_smac_data(args, batch_size=1, suffix="", datadir="data"):

    # load files
    obs_arr = np.load(datadir + "/obs_filt_" + suffix + "_sample.npy")[:, :, :, [4]]
    rewards_arr = np.load(datadir + "/r_filt_" + suffix + "_sample.npy")
    edges_file = np.load(datadir + "/gt_edges_" + suffix + "_sample.npy")
...
def load_lj_data(args, batch_size=1, suffix="", datadir="data"):
    ...
    # zero pad
    rewards_arr = np.pad(rewards_arr, ((0, 0), (0, 0), (0, 0), (0, 17)))
...
def load_springs_data(args, batch_size=1, suffix="", datadir="data"):
    ...
    loc_train = np.load(os.path.join(datadir, "loc_train" + suffix + ".npy"))#[:, :, :, [0, 2]]
    vel_train = np.load(os.path.join(datadir, "vel_train" + suffix + ".npy"))#[:, :, :, [0, 2]]
    edges_train = np.load(os.path.join(datadir, "edges_train" + suffix + ".npy"))#[:, :, [0, 2]][:, [0, 2], :]

    loc_valid = np.load(os.path.join(datadir, "loc_valid" + suffix + ".npy"))#[:, :, :, [0, 2]]
    vel_valid = np.load(os.path.join(datadir, "vel_valid" + suffix + ".npy"))#[:, :, :, [0, 2]]
    edges_valid = np.load(os.path.join(datadir, "edges_valid" + suffix + ".npy"))#[:, :, [0, 2]][:, [0, 2], :]

    loc_test = np.load(os.path.join(datadir, "loc_test" + suffix + ".npy"))#[:, :, :, [0, 2]]
    vel_test = np.load(os.path.join(datadir, "vel_test" + suffix + ".npy"))#[:, :, :, [0, 2]]
    edges_test = np.load(os.path.join(datadir, "edges_test" + suffix + ".npy"))#[:, :, [0, 2]][:, [0, 2], :]
