...
def forward_pass_and_eval(
    ...
    if args.use_encoder:
        if args.unobserved > 0 and args.model_unobserved == 0:
            ...
            # uncomment to save files for additional metrics
            #if epoch != None and epoch % 10 == 0:
            #    if epoch == 150:
            #        np.save(f"inputs_pp_at_{epoch}_epochs_batch_{batch_idx}.npy", data_encoder.cpu().numpy())
            #    else:
            #        np.save(f"inputs_pp_at_{epoch}_epochs.npy", data_encoder.cpu().numpy())

            # [n_samples, n_agents, n_timesteps, n_dim]
    ...
    losses["acc"] = utils.edge_accuracy(logits, relations, epoch=epoch, batch_idx=batch_idx)
